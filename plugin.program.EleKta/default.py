...
AUTOFEQ          = wiz.getS('autocleanfreq')
...
def index():
	if AUTOUPDATE == 'Yes':
		if wiz.workingURL(WIZARDFILE) == True:
			...
			if ver > VERSION: addFile('%s [v%s] [COLOR cyan][B][UPDATE v%s][/B][/COLOR]' % (ADDONTITLE, VERSION, ver), 'wizardupdate', themeit=THEME2)
	...
	if len(BUILDNAME) > 0:
		...
		if version > BUILDVERSION: build = '%s [COLOR cyan][B][UPDATE v%s][/B][/COLOR]' % (build, version)
	...
	addDir ('[COLOR cyan][B]-=[/B][/COLOR][COLOR mediumorchid] Builds [/COLOR][COLOR cyan][B]=-[/B][/COLOR]'        ,'builds',   icon=ICONBUILDS,   themeit=THEME1)
	addDir ('[COLOR cyan][B]-=[/B][/COLOR][COLOR mediumorchid] Maintenance [/COLOR][COLOR cyan][B]=-[/B][/COLOR]'   ,'maint',    icon=ICONMAINT,    themeit=THEME1)
	addDir ('[COLOR cyan][B]-=[/B][/COLOR][COLOR mediumorchid] Internet Tools [/COLOR][COLOR cyan][B]=-[/B][/COLOR]' ,'net', icon=ICONSPEED, themeit=THEME1)
	if wiz.platform() == 'android' or DEVELOPER == 'true': addDir ('[COLOR cyan][B]-=[/B][/COLOR][COLOR mediumorchid] Android [/COLOR][COLOR cyan][B]=-[/B][/COLOR]' ,'apk', icon=ICONAPK, themeit=THEME1)
	if not ADDONFILE == 'http://': addDir ('Addons' ,'addons', icon=ICONADDONS, themeit=THEME1)
	addDir ('[COLOR cyan][B]-=[/B][/COLOR][COLOR mediumorchid] Save Account Info [/COLOR][COLOR cyan][B]=-[/B][/COLOR]'     ,'savedata', icon=ICONSAVE,     themeit=THEME1)
	if HIDECONTACT == 'No': addFile('[COLOR cyan][B]-=[/B][/COLOR][COLOR mediumorchid] Wizard Support [/COLOR][COLOR cyan][B]=-[/B][/COLOR]' ,'contact', icon=ICONCONTACT,  themeit=THEME1)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	addFile('[COLOR cyan][B]-=[/B][/COLOR][COLOR mediumorchid] Settings [/COLOR][COLOR cyan][B]=-[/B][/COLOR]'      ,'settings', icon=ICONSETTINGS, themeit=THEME1)
	if DEVELOPER == 'true': addDir('[COLOR cyan][B]-=[/B][/COLOR][COLOR mediumorchid] Developer Menu [/COLOR][COLOR cyan][B]=-[/B][/COLOR]','developer', icon=ICONSETTINGS, themeit=THEME1)
...
def maintMenu(view=None):
	...
	if view == "misc" or SHOWMAINT == 'true': 
		addFile('Enable All Add-ons',                    'kodi17fix',       icon=ICONMAINT, themeit=THEME3)
	...
	if type == 'gui':
		...
		if installtheme:
			...
			if url not in ["fresh", "normal"]:
				test = testTheme(lib) if not wiz.currSkin() in ['skin.confluence', 'skin.estouchy'] else False
				test2 = testGui(lib) if not wiz.currSkin() in ['skin.confluence', 'skin.estouchy'] else False
				if test == True:
					wiz.lookandFeelData('save')
					skin     = 'skin.confluence' if KODIV < 17 else 'skin.estouchy'
...
def restoreit(type):
	...
	if not wiz.currSkin() in ['skin.confluence', 'skin.estouchy']:
		wiz.skinToDefault()
...
def net_tools(view=None):
	...
	setView('files', 'viewType')
def speedMenu():
	xbmc.executebuiltin('Runscript("special://home/addons/plugin.program.EleKta/speedtest.py")')
######################################################################################################################
######################################################################################################################			
...
def setView(content, viewType):
	if wiz.getS('auto-view')=='true':
		...
		if views == '50' and KODIV >= 17 and SKIN == 'skin.estouchy': views = '55'
		if views == '500' and KODIV >= 17 and SKIN == 'skin.estouchy': views = '50'
